...
{
    /// <summary>
    /// Block to build a list of people who should receive a review request.
    /// </summary>
    [DisplayName("Send Review Request")]
    [Category("CRM > ReviewRequest")]
    [Description("Block for selecting criteria to build a list of people who should receive a review request.")]
    [CommunicationTemplateField("Review Request Template", "The template to use with this block to send requests.", true, "B9A0489C-A823-4C5C-A9F9-14A206EC3B88")]
    ...
    public partial class ReviewSendRequest : Rock.Web.UI.RockBlock
    {
        #region Fields
        ...
        {
            if ( string.IsNullOrWhiteSpace(GetAttributeValue("ReviewRequestTemplate")) )
            {
                throw new Exception("Missing communication template configuration.");
            }

            var template = new CommunicationTemplateService(new RockContext()).Get(GetAttributeValue("ReviewRequestTemplate").AsGuid());
            if ( template != null )
            {
